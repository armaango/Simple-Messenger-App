...
public class SimpleMessengerActivity extends Activity {
    ...
    private class ServerTask extends AsyncTask<ServerSocket, String, Void> {
        ...
        protected Void doInBackground(ServerSocket... sockets){
            ...
            try {
                while(true) {
                    Socket server = serverSocket.accept();// http://download.java.net/jdk7/archive/b123/docs/api/java/net/ServerSocket.html
                    // Used DataInputStream and the related function readUTF() from http://developer.android.com/reference/java/io/DataInputStream.html
                    DataInputStream dataInputStream = new DataInputStream(server.getInputStream());
                    String msg = dataInputStream.readUTF();
                    //System.out.println("Received msg is " + msg);// To debug if correct message is being received before displaying
                    publishProgress(msg);// sending data to publishProgress
    ...
    private class ClientTask extends AsyncTask<String, Void, Void> {
        ...
        protected Void doInBackground(String... msgs) {
            try {
               ...
               //System.out.println("Message being sent is" + msgToSend);// checking if the data is being passed correctly
                DataOutputStream dataOutputStream=new DataOutputStream(socket.getOutputStream());
                // Used the DataOutputStream class and its functions writeUTF() and flush() from http://developer.android.com/reference/java/io/DataOutputStream.html
                dataOutputStream.writeUTF(msgToSend);
                dataOutputStream.flush();
                //int count=dataOutputStream.size(); //Code to debug the amount of sent data
               // System.out.println("The size written is"+count);
